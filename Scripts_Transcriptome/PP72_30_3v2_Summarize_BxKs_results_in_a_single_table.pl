...
PP72_30_3v2_Summarize_results_in_a_single_table.pl

SYNOPSIS

perl PP72_30_3v2_Summarize_results_in_a_single_table.pl  --pattern=Final
...
foreach my $f (nsort @files) {
  ...
  foreach my $line (<IN1>) {
    ...
    } elsif ($line =~ /Number\ of\ genes\ with\ a\ blastx\ match/) {
      @interm = split(/\t/,$line);
      $record{$temp}{blastxmatch} = $interm[1];           
...
warn"##############################################################################################################\n";
warn"#####....................................................................................................#####\n";
warn"                                            Make calculations\n";
warn"#####....................................................................................................#####\n";
warn"##############################################################################################################\n";

foreach my $sample (keys %record) {
  $record{$sample}{geneswithoutvariant} = $record{$sample}{blastxmatch} - $record{$sample}{geneswithvariants};
  
  $record{$sample}{percent_Genes_with_variants} = sprintf("%.4f", $record{$sample}{geneswithvariants} / $record{$sample}{blastxmatch}*100);
}
...
print OUT1 "Total number of genes with a blastx match after filtration";
foreach my $sample (nsort keys %record) {
  print OUT1 "\t", $record{$sample}{blastxmatch};
}
print OUT1 "\n";

print OUT1 "Number of genes without variants";
foreach my $sample (nsort keys %record) {
  print OUT1 "\t", $record{$sample}{geneswithoutvariant};
}
print OUT1 "\n";
...
print OUT1 "Percentage of genes with variants";
foreach my $sample (nsort keys %record) {
  print OUT1 "\t", $record{$sample}{percent_Genes_with_variants};
}
print OUT1 "\n";
